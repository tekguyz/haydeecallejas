...
Live at `https://haydeecallejas.vercel.app` — this was **not** a project rename (renaming a Vercel project does not automatically move or create domains). The domain changed 2026-07-09 via Vercel dashboard → Settings → Domains: added `haydeecallejas.vercel.app` directly, then deleted the old `haydee-callejas-site.vercel.app` domain (now 404s, confirmed). `metadataBase` in `app/layout.tsx` updated to match and redeployed. This project is now a local git repository (`git init` run 2026-07-09, no commits made yet).
