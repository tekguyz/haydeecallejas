...
- **WhatsApp deep link format:** `https://wa.me/50589000408?text=` + URL-encoded message including product name, selected color, and price. Test on an actual mobile device that the pre-filled message opens correctly. **A multi-product cart is being added** (users can select more than one product/color); once it ships, the CTA message will itemize every selected product with individual prices plus a total, instead of firing one message per product — this per-product format is what's live until the cart lands.
